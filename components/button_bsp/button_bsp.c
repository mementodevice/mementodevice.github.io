...
void user_button_init(void)
{
	...
	button_attach(&button1,BTN_DOUBLE_CLICK,on_boot_double_press);            		    //双击事件 (move back)
	...
	button_attach(&button2,BTN_DOUBLE_CLICK,on_pwr_double_press);            		    //双击事件 (help)
...
static void on_boot_double_press(Button* btn_handle)
{
	xEventGroupSetBits(boot_groups,set_bit_button(3));   /* BOOT double -> move back */
}
static void on_pwr_double_press(Button* btn_handle)
{
	xEventGroupSetBits(pwr_groups,set_bit_button(1));    /* PWR double -> help */
}
