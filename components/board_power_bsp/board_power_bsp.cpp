...
void board_power_bsp_t::EnableDeepLowPowerMode(uint32_t timer_wake_sec) {
    ...
    /* optional timer wake (enable AFTER disabling all sources above, or it gets cleared) */
    if (timer_wake_sec > 0) {
        ESP_ERROR_CHECK(esp_sleep_enable_timer_wakeup((uint64_t)timer_wake_sec * 1000000ULL));
    }
