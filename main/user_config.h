...
/* ===================== Memento dead-man's-switch (DMS) config ===================== */
/* Vault mount point. Despite the name it is the internal-flash 'storage' FAT partition
 * (no SD card) -- see user_app.cpp mount_internal_vault(). Paths, PIN length, KDF rounds
 * and per-PIN countdowns live in the dms_* components (dms_vault.c, dms_crypto.h), not here. */
#define DMS_SD_MOUNT        "/sdcard"
