...
/* When set, the next LVGL flush does a FULL-waveform refresh (clears e-ink ghosting/shadows)
 * instead of the usual fast partial update. Set by user_app (host->display_full_refresh). */
volatile bool g_epd_full_refresh = false;
...
static void example_lvgl_flush_cb(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
  	...
  	bool full = g_epd_full_refresh;
  	if (full) { g_epd_full_refresh = false; driver->EPD_Init(); }   /* full mode + full LUT */
    driver->EPD_Clear();
  	...
  	if (full) {
  	 	driver->EPD_DisplayPartBaseImage();   /* full-waveform refresh clears ghosting + sets base */
  	 	driver->EPD_Init_Partial();           /* restore partial mode for subsequent updates       */
  	} else {
